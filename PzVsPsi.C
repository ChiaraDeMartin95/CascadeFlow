...
Double_t fsine(Double_t *x, Double_t *par)
{
  return par[0] + 2 * par[1] * sin(x[0]);
}

Float_t YLow[numPart] = {-0.012};
Float_t YUp[numPart] = {0.012};

Int_t colorJunlee = kAzure - 3;
Int_t ColorOO = kMagenta + 1;

void PzVsPsi(Int_t ChosenPart = ChosenParticle,
             ...
             Bool_t UseTwoGauss = ExtrUseTwoGauss)
{
  ...
  Float_t UpperRangeParticle = 2 * TMath::Pi();
  Float_t LowerRangeParticle = 0;
  ...
  stringout = "../PzVsPsi/" + NameAnalysis[!isV2] + "_";
  ...
  LegendTitle->AddEntry("", "O-O, #sqrt{#it{s}_{NN}} = 5.36 TeV", "");
  if (isPolFromLambda)
  {
    if (isRapiditySel)
  ...
  fHistPzs = (TH1F *)fileIn->Get("histoPz" + sPolFromLambda[isPolFromLambda] + V2FromFit[isFromFit]);
  if (isGlobalFit)
    fHistPzs = (TH1F *)fileIn->Get("histoPz" + sPolFromLambda[isPolFromLambda] + "Global" + V2FromFit[isFromFit]);
  fHistPzsPsiInt = (TH1F *)fileIn->Get("histoPzPtInt" + sPolFromLambda[isPolFromLambda] + V2FromFit[isFromFit]);
  if (isGlobalFit)
    fHistPzsPsiInt = (TH1F *)fileIn->Get("histoPzPtInt" + sPolFromLambda[isPolFromLambda] + "Global" + V2FromFit[isFromFit]);
  ...
  if (!fHistPzsPsiInt)
  {
    cout << " no hist v2 / Pzs PsiInt" << endl;
    return;
  }
  if (!fHistPurity)
  {
  ...
  fHistPzsBkgSub = (TH1F *)fHistPzs->Clone("fHistPzsBkgSub");
  if (!isFromFit) // NOTE: if from fit, the background is already subtracted as a function of the invariant mass before fitting
  {
    for (Int_t b = 1; b <= fHistPzs->GetNbinsX(); b++)
    {
      fHistPzsBkgSub->SetBinContent(b, fHistPzs->GetBinContent(b) - fHistPzsPsiInt->GetBinContent(1));
      fHistPzsBkgSub->SetBinError(b, TMath::Sqrt(fHistPzs->GetBinError(b) * fHistPzs->GetBinError(b) + fHistPzsPsiInt->GetBinError(1) * fHistPzsPsiInt->GetBinError(1)));
    }
  }

  // Get histogram with syst uncertainty
  TString PathInSyst = "../Systematics/SystVsPsi_" + NameAnalysis[!isV2] + "_";
  ...
  canvasPzs->cd();
  SetFont(hDummy);
  StyleHistoYield(hDummy, YLow[part], YUp[part], 1, 1, TitleXPsi, TitleYPz, "", 1, 1.15, 1.6);
  StyleHistoYield(fHistPzs, YLow[part], YUp[part], kBlack, MarkerPart[part], TitleXPsi, TitleYPz, "", MarkerPartSize[part], 1.15, 1.6);
  StyleHistoYield(fHistPzsBkgSub, YLow[part], YUp[part], ColorPart[part], MarkerPart[part], TitleXPsi, TitleYPz, "", MarkerPartSize[part], 1.15, 1.6);
  StyleHistoYield(fHistPzsPsiInt, YLow[part], YUp[part], kViolet, MarkerPart[part], TitleXPsi, TitleYPz, "", MarkerPartSize[part], 1.15, 1.6);
  StyleHistoYield(fHistPzsSist, YLow[part], YUp[part], ColorPart[part], MarkerPart[part], TitleXPsi, TitleYPz, "", 1.5, 1.15, 1.6);
  ...
  if (ChosenPart >= 6)
  {
    ...
    fHistPzsBkgSub->SetLineColor(ColorOO);
    fHistPzsBkgSub->SetMarkerColor(ColorOO);
  }
  TF1 *lineAtZero = new TF1("pol0Psi", "0", LowerRangeParticle, UpperRangeParticle);
  lineAtZero->SetLineColor(kBlack);
  lineAtZero->SetLineStyle(2);
  lineAtZero->Draw("same");
  fHistPzsBkgSub->Draw("same ex0");
  TF1 *fitsine = new TF1("sine", fsine, LowerRangeParticle, UpperRangeParticle, 2);
  fitsine->SetLineColor(ColorOO);
  fitsine->SetLineWidth(2);
  fHistPzsBkgSub->Fit(fitsine, "R");
  fitsine->Draw("same");
  TLegend *legendsinefit = new TLegend(0.2, 0.18, 0.6, 0.38);
  legendsinefit->SetFillStyle(0);
  legendsinefit->SetTextSize(0.04);
  legendsinefit->AddEntry(fitsine, "p0 + 2*p1 sin(2#varphi - 2#Psi_{2})", "l");
  legendsinefit->AddEntry("", Form("p0 = %.4f +- %.4f", fitsine->GetParameter(0), fitsine->GetParError(0)), "");
  legendsinefit->AddEntry("", Form("p1 = %.4f +- %.4f", fitsine->GetParameter(1), fitsine->GetParError(1)), "");
  legendsinefit->Draw("same");
  fHistPzsSist->SetFillStyle(0);
  fHistPzsSist->Draw("same e2");
  LegendTitle->Draw("");
  canvasPzs->SaveAs("../PzVsPsi.pdf");
  canvasPzs->SaveAs("../PzVsPsi.png");
  canvasPzs->SaveAs("../PzVsPsi.eps");
  ...
  canvasPzsError->cd();
  SetFont(hDummyError);
  StyleHistoYield(hDummyError, 0, 0.004, 1, 1, TitleXPsi, "Absolute uncertainty", "", 1, 1.15, 1.6);
  if (part == 1)
    hDummyError->GetYaxis()->SetRangeUser(0, 0.015);
  StyleHistoYield(fHistPzsError, 0, 0.01, ColorPart[part], MarkerPart[part], TitleXPsi, "", "", MarkerPartSize[part], 1.15, 1.6);
  StyleHistoYield(fHistPzsSistError, 0, 0.01, kGray + 2, 22, TitleXPsi, "", "", MarkerPartSize[part], 1.15, 1.6);
  ...
  StyleHistoYield(hDummySignif, 0, 1.2 * fHistPzsSignif->GetBinContent(fHistPzsSignif->GetMaximumBin()), 1, 1, TitleXPsi, "S / #sigma_{S}", "", 1, 1.15, 1.6);
  StyleHistoYield(fHistPzsSignif, 0, 1.2 * fHistPzsSignif->GetBinContent(fHistPzsSignif->GetMaximumBin()), ColorPart[part], MarkerPart[part], TitleXPsi, "S / #sigma_{S}", "", MarkerPartSize[part], 1.15, 1.6);
  StyleHistoYield(fHistPzsSignifStat, 0, 1.2 * fHistPzsSignif->GetBinContent(fHistPzsSignif->GetMaximumBin()), kGray + 2, 22, TitleXPsi, "S / #sigma_{S}", "", MarkerPartSize[part], 1.15, 1.6);
  StyleHistoYield(fHistPzsSignifLambda, 0, 1.2 * fHistPzsSignif->GetBinContent(fHistPzsSignif->GetMaximumBin()), kBlue, 20, TitleXPsi, "S / #sigma_{S}", "", MarkerPartSize[part], 1.15, 1.6);
  ...
  hDummyPurity->Draw("");
  StyleHistoYield(fHistPurity, 0.9, 1, ColorPart[part], MarkerPart[part], TitleXPsi, "S / (S+B)", "", MarkerPartSize[part], 1.15, 1.6);
  ...
  StyleHistoYield(fHistSignificance, 0, 1.2 * fHistSignificance->GetBinContent(1), ColorPart[part], MarkerPart[part], TitleXPsi, "S / #sqrt{S+B}", "", MarkerPartSize[part], 1.15, 1.6);
  ...
  hDummyYield->Draw("");
  StyleHistoYield(fHistYield, 0, 0.015, ColorPart[part], MarkerPart[part], TitleXPsi, "Yield", "", MarkerPartSize[part], 1.15, 1.6);
  ...
  hDummyB->Draw("");
  StyleHistoYield(fHistB, 0, 0.1, ColorPart[part], MarkerPart[part], TitleXPsi, "B / N_{events}", "", MarkerPartSize[part], 1.15, 1.6);
  fHistB->Draw("same");
  ...
  hDummyTot->Draw("");
  StyleHistoYield(fHistTot, 0, 1, ColorPart[part], MarkerPart[part], TitleXPsi, "(S+B) / N_{events}", "", MarkerPartSize[part], 1.15, 1.6);
  fHistTot->Draw("same");
  ...
  hDummySigma->Draw("");
  StyleHistoYield(fHistMean, 1.31, 1.33, ColorPart[part], MarkerPart[part], TitleXPsi, "#mu", "", MarkerPartSize[part], 1.15, 2);
  fHistMean->Draw("same");
  ...
  canvasPzsXi->cd();
  SetFont(hDummy);
  StyleHistoYield(hDummy, YLow[part], YUp[part], 1, 1, TitleXPsi, TitleYPz, "", 1, 1.15, 1.8);
  StyleHistoYield(fHistPzsBkgSub, YLow[part], YUp[part], kOrange + 10, 20, TitleXPsi, TitleYPz, "", 1.9, 1.15, 1.8);
  StyleHistoYield(fHistPzsSist, YLow[part], YUp[part], kOrange + 10, 20, TitleXPsi, TitleYPz, "", 1.9, 1.15, 1.8);
  SetHistoTextSize(hDummy, xTitle, xLabel, xOffset, xLabelOffset, yTitle, yLabel, yOffset, yLabelOffset);
  SetTickLength(hDummy, tickX, tickY);
  hDummy->GetXaxis()->SetRangeUser(LowerRangeParticle, UpperRangeParticle);
  hDummy->Draw("");
  ...
  fHistPzsBkgSub->DrawClone("same ex0");
  fHistPzsSist->SetFillStyle(0);
  ...
  hDummy->Draw("");
  lineatZero->Draw("same");
  TH1F *fHistPzsUpTo50 = (TH1F *)fHistPzsBkgSub->Clone("fHistPzsUpTo50");
  ...
  cout << "\nFit with sine: " << endl;
  cout << "p0 = " << fitsine->GetParameter(0) << " +- " << fitsine->GetParError(0) << endl;
  cout << "p1 = " << fitsine->GetParameter(1) << " +- " << fitsine->GetParError(1) << endl;
  cout << "Chi2/NDF = " << fitsine->GetChisquare() << "/" << fitsine->GetNDF() << endl;

  cout << "\nAvergae of Pz in the six azimuthal bins: " << endl;
  cout << "Pz = " << fHistPzs->GetMean() << "+-" << fHistPzs->GetMeanError() << endl;
  float mean = 0;
  float meanerr = 0;
  for (Int_t b = 1; b <= fHistPzs->GetNbinsX(); b++)
  {
    mean += fHistPzs->GetBinContent(b);
    meanerr += fHistPzs->GetBinError(b);
  }
  mean /= fHistPzs->GetNbinsX();
  meanerr /= fHistPzs->GetNbinsX();
  cout << "Mean of Pz in the six azimuthal bins: " << mean << " +- " << meanerr << endl;
